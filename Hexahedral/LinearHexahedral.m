clear; [E,v,rho,g,coord,etopol,bc,f]=inputFE3; stressfac=1e-10;
nodes=size(coord,1); nDoF=3*nodes; nels=size(etopol,1); eDoF=zeros(24,1);
DoF=reshape(1:nDoF,3,nodes)'; K=zeros(nDoF); d=zeros(nDoF,1); Gpsig=[];
Iz=zeros(6); Iz(1:3,1:3)=ones(3); I2=eye(6); I2(4:6,4:6)=0.5*eye(3);
D=E/((1+v)*(1-2*v))*(v*Iz+((1-2*v)*I2));
wp=ones(8,1); dN=zeros(24,8); Gpcoord=zeros(nels,8,3); BeGp=zeros(nels,8,6,24);
...
for nel=1:nels
  ...
  for i=1:8; indx=[(3*i)-2:3*i]';
    ...
    detJ=det(JT(indx,:)); dNx=(JT(indx,:))\dN(indx,:); B=zeros(6,24);
    ...
    N3b24=[N(i,1)*eye(3) N(i,2)*eye(3) N(i,3)*eye(3) N(i,4)*eye(3) ...
           N(i,5)*eye(3) N(i,6)*eye(3) N(i,7)*eye(3) N(i,8)*eye(3)];
    f(eDoF)=f(eDoF)+(N3b24'*[0 0 (-rho*g)]'*detJ*wp(i));
  end
  K(eDoF,eDoF)=K(eDoF,eDoF)+ke;
end
fDoF=[1:nDoF]'; pDoF=bc(:,1); d(pDoF)=bc(:,2); fDoF(pDoF)=[];
d(fDoF)=K(fDoF,fDoF)\(f(fDoF)-K(fDoF,pDoF)*d(pDoF)); uvw=reshape(d,3,nodes)'; %uvw= x y z displacement Graphical from here
