[coord,etpl,fext0,bc,lam,miu,ngp,lstps,NRitmax,NRtol]=InputData; ndim=3;
[nels,nen]=size(etpl); nodes=size(coord,1); nDoF=nodes*ndim; neDoF=(nen*ndim)^2;
...
for lstp=0:lstps
  ...
  while ((NRit<NRitmax)&&(oobfnorm>NRtol))
    ...
    for nel=1:nels
      ...
      [ke,felem,epsE(:,:,nel),sig(:,:,nel),F(:,:,:,nel)]=FE(coord(etpl(nel,:),:),lam,miu,uvw(ed),duvw(ed),ngp,Fold(:,:,:,nel));
